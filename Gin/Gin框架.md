...
> 首先定义一个存放模板文件的`templates`文件夹，然后在其内部按照业务分别定义一个`posts`文件夹和一个`users`文件夹，各有一个index.html文件
...
func main() {
	r := gin.Default()
	r.Static("/static", "./static")//指定css，js等静态文件位置
	r.LoadHTMLGlob("templates/**/*")//指定html文件位置
   // ...
	r.Run(":8080")
}
```
